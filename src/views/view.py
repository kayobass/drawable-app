...
class DrawableView:
    ...
    def criar_widgets_selecao(self):
        ...
        :return: None
        """
        self.label = ttk.Label(self.frame, text="Escolha a ferramenta:")
        self.label.grid(row=0, column=0, sticky="w", **self.paddings)
        self.tipo_figura = StringVar(value="Linha")
        self.option_menu = ttk.OptionMenu(
            self.frame, self.tipo_figura, self.figuras[1], *self.figuras
        )
        self.option_menu.grid(row=0, column=1, sticky="w", **self.paddings)
        self.label_lados = ttk.Label(self.frame, text="  Lados:")
        self.label_lados.grid(row=0, column=2, sticky="w", **self.paddings)
        ...
        self.combo_lados.grid(row=0, column=3, sticky="w", **self.paddings)

        self.botao_salvar = Button(self.frame, text="Salvar", state="disabled")
        self.botao_salvar.grid(row=0, column=4, sticky="e", padx=(5, 0), pady=5)

        self.botao_carregar = Button(self.frame, text="Carregar")
        self.botao_carregar.grid(row=0, column=5, sticky="e", padx=(5, 5), pady=5)
    ...
    def criar_widgets_personalizacao(self):
        ...
        :return: None
        """
        self.label_cores = ttk.Label(self.frame, text="Escolha as cores do desenho:")
        self.label_cores.grid(row=1, column=0, sticky="w", **self.paddings)
        self.frame_cores = Frame(self.frame)
        self.frame_cores.grid(
            row=1, column=1, columnspan=5, sticky="w", **self.paddings
        )
        self.botao_cor_borda = Button(self.frame_cores, text="Cor da borda")
        ...
        self.botao_cor_preenchimento = Button(
            self.frame_cores, text="Cor do preenchimento"
        )
        ...
        self.botao_sem_preenchimento = Button(
            self.frame_cores, text="Sem preenchimento"
        )
    ...
    def criar_area_desenho(self):
        ...
        self.canvas.grid(row=2, column=0, columnspan=6, sticky="nsew", **self.paddings)
