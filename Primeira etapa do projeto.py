...
def desenhar_figuras():
    canvas.delete("all")
    for fig, values, cor, cor_preenchimento in figuras:
        if fig == "linha":
...
def desenhar_figura_nova():
    ...
    if fig == "linha":
        ...
        canvas.create_rectangle(values[0], values[1], values[2], values[3], dash=(4, 2), outline= cor_da_borda, fill= cor_do_preenchimento)
    elif fig == "oval":
        canvas.create_oval(values[0], values[1], values[2], values[3], dash=(4, 2), outline= cor_da_borda, fill= cor_do_preenchimento)
    elif fig == "circulo":
        a,b,c,d = oval_em_circulo(values)
